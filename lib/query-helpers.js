module.exports = function(knex) {
  ...
  return {
    ...
    selectUsersTableByPollId : (poll_id, done) => {
      knex.select().table('polls').where('id', poll_id).then((results) => {
        return knex.select().from('users').where('id', results[0].user_id);
      }).then((results) => {
        done(results[0]);
      }).catch((err) => {
        console.error(err);
    ...
    selectPollsRetrieveAdmin: (poll_id, done) => {
      knex.select('admin_key').table('polls').where('id', poll_id).then((results) => {
        done(results[0]);
      }).catch((err) => {
        console.error(err);
      })
